      ...
      <div className="bg-[var(--ink)] rounded-lg p-8 shadow-sm border border-[var(--ink)]">
        ...
        <div className="flex flex-col lg:flex-row items-stretch gap-8 border-t border-[#333] pt-8">
          ...
          <div className="w-full lg:w-[45%] flex flex-col lg:pr-8 lg:border-r border-[#333]">
            <Link href={`/sports/${heroPost.slug}`} className="group block mb-6 transition-all duration-200 ease-in-out hover:bg-white/5 hover:shadow-lg hover:scale-[1.01] rounded-xl p-4 -mx-4 -mt-4">
              {heroPost.bannerImage ? (
                <Image src={optimizeCloudinaryUrl(heroPost.bannerImage, { width: 600, crop: 'fill' })} alt={heroPost.title} width={600} height={337} className="w-full aspect-[16/9] object-cover mb-4" />
              ) : (
              ...
              <div className="bg-[#111] border-t-[3px] border-t-[var(--green)] border border-[#333] rounded-sm py-4 px-5 shadow-sm">
                ...
                <div className="flex flex-col">
                  {localPetitions.map(petition => {
                    ...
                    const hasSignedLocal = typeof window !== 'undefined' && localStorage.getItem(`signed_petition_${petition._id}`);
