    ...
    <section className="mb-12 font-[var(--font-ui)]">
      ...
      <div className="flex flex-col lg:flex-row items-stretch gap-6 border-t border-[var(--line)] pt-8">
        ...
        <div className="w-full lg:w-[65%] flex flex-col lg:pr-6 pb-6 lg:pb-0 lg:border-r border-[var(--line)]">
          {largePost && (
            <Link href={`/${largePost.vertical?.slug || vertical.slug}/${largePost.slug}`}
            ...
            >
              ...
              <div className="w-full md:w-[55%] shrink-0">
                {largePost.bannerImage ? (
                  <Image src={optimizeCloudinaryUrl(largePost.bannerImage, { width: 800, crop: 'fill' })} alt={largePost.title} width={800} height={450} className="w-full aspect-[16/9] object-cover rounded-sm" />
                ) : (
            ...
            <div className="pt-6 border-t border-[var(--line)]">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {mediumPosts.map(post => (
                  <Link key={post._id} href={`/${post.vertical?.slug || vertical.slug}/${post.slug}`}
                  ...
                  >
                    <div className="flex gap-4">
                      <div className="w-[120px] shrink-0">
                        {post.bannerImage ? (
                          <Image src={optimizeCloudinaryUrl(post.bannerImage, { width: 300, crop: 'fill' })} alt={post.title} width={300} height={225} className="w-full aspect-[4/3] object-cover rounded-sm" />
                        ) : (
          ...
          {gridPosts.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-stretch">
              {gridPosts.map((post) => (
                ...
                >
                  {post.bannerImage ? (
                    <Image src={optimizeCloudinaryUrl(post.bannerImage, { width: 400, crop: 'fill' })} alt={post.title} width={400} height={300} className="w-full aspect-[4/3] object-cover mb-3 rounded-sm" />
                  ) : (
