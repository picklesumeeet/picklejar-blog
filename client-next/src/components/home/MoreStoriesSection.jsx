                  ...
                  <div key={post._id} className={!isFirst ? 'border-t border-[var(--line)] py-1' : 'pb-1 pt-0'}>
                    ...
                    >
                      {isFirst ? (
                        <div>
                          {post.bannerImage ? (
                            <Image src={optimizeCloudinaryUrl(post.bannerImage, { width: 600, crop: 'fill' })} alt={post.title} width={600} height={337} className="w-full aspect-[16/9] object-cover mb-3" />
                          ) : (
              ...
              {vertAPosts.length > 0 ? (
                <>
                  <div className="pb-5 mb-5 border-b border-[var(--line)]">
                    <Link href={`/${featuredVertA.slug}/${vertAPosts[0].slug}`} className="group block transition-all duration-200 ease-in-out hover:bg-gray-50 hover:shadow-md hover:scale-[1.01] rounded-xl p-3 -mx-3">
                      {vertAPosts[0].bannerImage ? (
                        <Image src={optimizeCloudinaryUrl(vertAPosts[0].bannerImage, { width: 600, crop: 'fill' })} alt={vertAPosts[0].title} width={600} height={337} className="w-full aspect-video object-cover mb-3" />
                      ) : (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    {vertAPosts.slice(1, 4).map(post => (
                      <Link key={post._id} href={`/${featuredVertA.slug}/${post.slug}`} className="group block transition-all duration-200 ease-in-out hover:bg-gray-50 hover:shadow-md hover:scale-[1.01] rounded-xl p-2 -mx-2">
                        {post.bannerImage ? (
                          <Image src={optimizeCloudinaryUrl(post.bannerImage, { width: 300, crop: 'fill' })} alt={post.title} width={300} height={225} className="w-full aspect-[4/3] object-cover mb-2" />
                        ) : (
            ...
            <div className="w-full flex justify-center">
              {ad ? (
                <a href={ad.ctaUrl || '#'} target="_blank" rel="noreferrer" className="block w-full max-w-[300px] h-[250px]">
                  {ad.image ? (
                    <Image src={ad.image} alt={ad.ctaText || 'Ad'} width={300} height={250} className="w-full h-full object-cover border border-[var(--line)]" />
                  ) : (
