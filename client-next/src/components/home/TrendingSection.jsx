    ...
    <section className="mb-12 font-[var(--font-ui)]">
      ...
      <div className="flex flex-col lg:flex-row items-stretch gap-6 border-t border-[var(--line)] pt-8">
        ...
        <div className="w-full lg:w-1/2 flex flex-col border-b lg:border-b-0 lg:border-r border-[var(--line)] lg:pr-6 pb-6 lg:pb-0">
          <Link href={`/${topPost.vertical?.slug || 'vertical'}/${topPost.slug}`} className="group block cursor-pointer transition-all duration-200 ease-in-out hover:bg-gray-50 hover:shadow-md hover:scale-[1.01] rounded-xl p-4 -mx-4 -mt-4">
            {topPost.bannerImage ? (
              <Image src={optimizeCloudinaryUrl(topPost.bannerImage, { width: 800, crop: 'fill' })} alt={topPost.title} width={800} height={450} className="w-full h-[280px] lg:h-[320px] object-cover mb-5 rounded-md" priority />
            ) : (
        ...
        <div className="w-full lg:w-1/4 flex flex-col border-b lg:border-b-0 lg:border-r border-[var(--line)] lg:pr-6 pb-6 lg:pb-0">
          <div className="border-b border-[var(--line)] pb-6 mb-6">
            <Link href={`/${midTopPost.vertical?.slug || 'vertical'}/${midTopPost.slug}`} className="group block cursor-pointer transition-all duration-200 ease-in-out hover:bg-gray-50 hover:shadow-md hover:scale-[1.01] rounded-xl p-3 -mx-3 -mt-3">
            {midTopPost.bannerImage ? (
              <Image src={optimizeCloudinaryUrl(midTopPost.bannerImage, { width: 400, crop: 'fill' })} alt={midTopPost.title} width={400} height={225} className="w-full aspect-video object-cover mb-4 rounded-md" />
            ) : (
          ...
          <div className="pb-6">
            <Link href={`/${midBottomPost.vertical?.slug || 'vertical'}/${midBottomPost.slug}`} className="group block cursor-pointer transition-all duration-200 ease-in-out hover:bg-gray-50 hover:shadow-md hover:scale-[1.01] rounded-xl p-3 -mx-3">
            <div className="mb-2">
              <span className="inline-block bg-[var(--green)] text-white px-2 py-0.5 rounded-full text-[9px] font-bold tracking-wider uppercase">
          ...
          <div className="flex flex-col">
            {displayLatest.length > 0 && (
              <div className="pb-6 pt-2">
                <Link href={`/${displayLatest[0].vertical?.slug || 'vertical'}/${displayLatest[0].slug}`}
